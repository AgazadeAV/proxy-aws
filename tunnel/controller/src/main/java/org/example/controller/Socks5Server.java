        ...
        public void channelRead(ChannelHandlerContext ctx, Object msg) {
            ...
            try {
                if (state == State.HANDSHAKE) {
                    ...
                    buf.readByte(); // VER (0x05)
                    int nMethods = buf.readByte() & 0xFF;
                    if (buf.readableBytes() < nMethods) return;
                    buf.skipBytes(nMethods);
                    ctx.writeAndFlush(Unpooled.wrappedBuffer(new byte[]{0x05, 0x00})); // no-auth
                    state = State.REQUEST;

                } else if (state == State.REQUEST) {
                    if (buf.readableBytes() < 4) return;
                    buf.readByte(); // VER
                    int cmd = buf.readByte() & 0xFF;  // 0x01 CONNECT
                    buf.readByte(); // RSV
                    ...
                    // Ответ клиенту отправим из onConnectAck (успех/фейл)
                    state = State.STREAM;
